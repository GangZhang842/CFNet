...
class RVNet(nn.Module):
    def __init__(self, base_block, context_layers, layers, double_branch=True):
        super(RVNet, self).__init__()
        self.double_branch = double_branch
        #encoder
        self.header = self._make_layer(eval('backbone.{}'.format(base_block)), context_layers[0], context_layers[1], layers[0], stride=(1, 2), dilation=1)
        ...
        if self.double_branch:
            self.sem_ins_conv = nn.Sequential(
                backbone.conv3x3(self.out_channels, 2 * self.out_channels, stride=1, dilation=1),
